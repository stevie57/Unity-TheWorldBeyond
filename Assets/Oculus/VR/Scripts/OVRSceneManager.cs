...
public class OVRSceneManager : MonoBehaviour
{
  ...
  // Maintain Scene Model entities currently loaded on the app space.
  private Dictionary<OVRSpace, GameObject> _sceneAnchors = new Dictionary<OVRSpace, GameObject>();
  ...
  private bool LoadSpatialEntities()
  {
    ...
    foreach (var kv in _sceneAnchors)
    {
      Destroy(kv.Value);
    }
    _sceneAnchors.Clear();
  ...
  private OVRSceneAnchor InstantiateSceneAnchor(OVRSpace space, Guid uuid, OVRSceneAnchor prefab)
  {
    ...
    _sceneAnchors[space] = sceneAnchor.gameObject;
